...
- **BREAKING** Reserve `RELKIT_ENV` for the framework and remove it from generated application schemas; pipelines provide different values for identical declared keys.
- **BREAKING** Separate deployment target/adapter selection into `relkit.config.ts` and version provider-aware graph, manifest, and deployment-plan contracts together.
...
- Migrate templates, examples, inspector, documentation, tests, and `my-relkit-app-7` to the new topology without compatibility shims.
...
- `cli-scaffolding`: Generate and validate single-topology applications without application-declared `RELKIT_ENV`.
