...
The compiler SHALL generate graph and manifest contracts with versions bumped together; the manifest SHALL contain executable handlers, provider adapter factories keyed independently by capability and adapter, middleware adapters, and request transforms plus the expected graph hash. Runtime activation SHALL fail on a version, hash, missing reference, or required-handler/factory mismatch.
