...
- [x] 1.2 Replace `ProviderSets` and provider recipes in `defineApp` with one topology and reserve `ZSYS_ENV` in environment schemas.
...
- [x] 3.5 Make `@zsys/testing` override all required bindings with deterministic fakes by default and support explicit configured-adapter opt-in.
...
- [x] 5.1 Migrate CLI generation/runtime activation, doctor checks, templates, commerce, and canonical fixtures to one topology without `ZSYS_ENV` schemas.
...
- [x] 5.4 Migrate `/Users/mustafaelsayed/Workspace/typescripts/my-zsys-app-7` to `external(s3(...))` and `external(redis(...))`, preserving unrelated user edits.
