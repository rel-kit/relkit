...
|   5 | `bun run scripts/pack-and-smoke-create-zsys.ts` |  `0` |        `30.411s` | packed create smoke passed with 27 packages                                       |
...
`ZSYS_AWS_INTEGRATION=0 bun run test:all` also passed every local layer,
...
`ZSYS_AWS_INTEGRATION=0` in the process environment. The checked-in `.env`
...
- Before `bun run verify`, the pre-existing generated
  `apps/fixture-commerce/.zsys` and `test-results` directories were moved to
  `/tmp/zsys-gate16-17.19.zdylhf/preserved-generated` and were restored after
...
- `openspec validate implement-zsys-typescript-poc-v3 --strict`: pass.
