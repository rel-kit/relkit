...
- **THEN** only the selected bucket binding is constructed from the one application topology, no environment-specific provider set is selected, and cache credentials or connectivity cannot affect readiness
...
- **WHEN** an adapter option references a field from the environment descriptor
