...
- Set `RELKIT_AWS_INTEGRATION=0` in the process before invoking the exact local
...
- Reuse the accepted AWS lifecycle evidence and run the local deployment suite
  with `RELKIT_AWS_INTEGRATION=0`; do not create another cloud stack for owner
  sign-off.
...
- Run the local deployment suite with `RELKIT_AWS_INTEGRATION=0`; retain the
...
- Treat generated `dist`, cache, and `.relkit` outputs as non-authored source;
...
- Scan the bounded release surfaces already produced by the repository:
  ...
  selected by `RELKIT_SECURITY_IMAGE`. Do not scan source files containing the
...
- Make the existing `create-relkit` package bin a real executable by keeping the
...
- Keep the documentation as five focused guides backed by existing commands
  ...
  `relkit` CLI instead of introducing documentation-only helpers or commands.
...
- Treat the fixture's telemetry wildcard diagnostic as an intentional warning;
  ...
  Scope scans exclude ignored `dist`, `node_modules`, `.turbo`, and `.relkit`
...
- Approve Gate 15 after rerunning the real release-gated AWS path end to end:
  ...
  passed for stack `relkit-nightly-1787062081840-d2826d4f`.
- Keep Pulumi state in Pulumi's backend model. Use the disposable S3 backend
  ...
  gate, and no parallel RelKit state system was added.
...
- Use a disposable copy of the full fixture and an explicit local Pulumi
  ...
  entry is the repository-equivalent `relkit` binary because no installed shell
...
- Gate the real-cloud test behind `RELKIT_AWS_INTEGRATION=1` and require an
...
- Keep stack destruction and cleanup verification in the finalization path.
  ...
  AWS Resource Groups Tagging API until no `managed-by=relkit`, app, and stack
...
- Add only the bounded `RELKIT_PROVIDER_READY_DELAY_MS` generated-server seam
...
- Persist only deterministic generated Pulumi files and command reports under
  `.relkit/generated/pulumi/`; reports use the existing redacted event/output
...
- Use the pinned Bun native `RedisClient` only behind a promise-only
  `RelkitValkeyClient`; do not add a Redis package or expose the native client
...
- Keep `RelkitEventBus` as the single AWS owner for the custom EventBridge bus,
...
- Keep `RelkitJobQueues` as the AWS owner for standard SQS source queues, DLQs,
...
- Derive component/resource names from the explicit app and stack identity,
  ...
  `managed-by=relkit`; source paths are never identity inputs.
- Keep the safe defaults concrete: two AZs with a single NAT gateway, an
  ...
  `/_relkit/v1/health/ready`, ECS liveness at `/_relkit/v1/health/live`, desired
...
- Reuse `@relkit/observability` redaction and the existing Effect logger layer;
...
- Keep all Pulumi-specific logging/report types in `@relkit/deploy-pulumi` and
  leave provider-neutral `@relkit/deploy` free of Pulumi/cloud executable types.
...
- Keep generated and inline modes on the same normalized plan/tag/name
  helpers. Generated output defaults to `.relkit/generated/pulumi` and contains
...
- Add direct `@relkit/contracts` and `@relkit/deploy` dependencies to the Pulumi
  owner for canonical serialization and the provider-neutral plan type. Do
  not add Pulumi/cloud types to `@relkit/deploy`.
...
- Default local development to a work-directory `.pulumi` backend and
  ...
  `.relkit/state` store; no custom state serialization or state lifecycle was
  added.
...
- Keep `server/index.ts` and `server/runtime.manifest.ts` in the local build for
  the existing `relkit start` path, but emit a minified `server/index.js` bundle
...
- Add the context allowlist and explicit `.env`/`.relkit/state`/
  `.relkit/observability` exclusions in the generated `.dockerignore`; retain
...
- Keep the plan converter split across focused private files to satisfy the
  repository's 200-line implementation limit. Add only `@relkit/graph` to the
  deploy workspace dependency because the converter consumes the public graph
  model/hash API.

# Task 16.2 deployment plan decisions

- Keep `@relkit/deploy` dependent only on the existing `@relkit/contracts` JSON
...
- Keep `@relkit/deploy` provider-neutral with no Pulumi/AWS dependency because
  Phase 15 plan contracts must not expose Pulumi executable types.
- Put `@pulumi/pulumi` in `@relkit/deploy-pulumi` for the Automation API and
  Pulumi program owner. Put `@pulumi/pulumi`, `@pulumi/aws`, and `@pulumi/awsx`
  in `@relkit/cloud-aws`; the reviewed ECS/Fargate/ALB component owner justifies
...
- Resolve the required root build command with a thin `scripts/build.ts`
  ...
  implemented `relkit build` artifact behavior.
...
- Expose one `GenerateProjectContext.failAt(point)` seam using the seven named
  ...
  `RELKIT_CREATE_<POINT>_FAILED` errors without adding a second command path.
...
- Do not make `create-relkit` depend on `@relkit/cli`: the CLI already depends on
  the generator. The generator invokes the installed `relkit` binary or the
...
- Use the current checked-in package/tool versions directly in each template:
  RelKit `0.0.0`, Bun `1.3.10`, and TypeScript `5.9.2`. The later generator and
  packed-release work owns substitution/release compatibility; this unit
  leaves no unresolved version marker behind.
- Keep generated application source on public `@relkit/app`, `@relkit/config`,
  `@relkit/schema`, and `@relkit/testing` seams, with global local/test/AWS
...
- Accept both separated and `--option=value` values for value-taking flags,
  ...
  context share the same normalized result as standalone `create-relkit`.
...
- Reuse the compiler's `loadConfig` validation for `relkit.config.ts` and load
...
- Reuse `@relkit/config` `projectEnv` and `resolveEnv` as the sole contract boundary. Load `src/env.ts` by default for the real project path, while accepting an injected definition/source for focused command tests; do not reimplement parsers or inspect runtime/provider state.
...
- Add the required `@relkit/config` workspace dependency to `@relkit/cli`, mark only `15.5` complete, and leave `15.6` as the next different unchecked unit.
...
- Keep graph commands artifact-only. Read `application.graph.json`, validate
  ...
  canonicalization, hashing, and compatibility classification to `@relkit/graph`.
...
- Let `create-relkit` own option normalization and generation. The CLI calls
  only `normalizeCreateOptions(args, { json })` and `generateProject(options,
context)`, so `relkit create` cannot drift from the standalone generator.
  The package is not implemented early; an unavailable API is reported as a
  structured `RELKIT_CREATE_API_UNAVAILABLE` failure until its later owner lands.
...
- Inspect live JSON requests/responses and request bodies for internal package,
  ...
  asserting a real `relkit.inspector` protocol payload is received. Keep the
...
- Keep event runtime data behind the existing versioned inspector runtime
  boundary. Query the local `relkit.events.admin` protocol through the generation
...
- Keep the browser client protocol-only and define the small JSON-safe
  ...
  `@relkit/inspector-api` package into `apps/inspector`. Graph/runtime responses
  use `relkit.inspector`, while request/log/trace pages use the distinct
  `relkit.observability.query` envelope and are validated explicitly.
...
- Mount the endpoint adapter with the existing Hono convention in
  `@relkit/inspector-api`, while keeping query parsing, redaction, pagination,
  cursor semantics, and stream retention in `@relkit/observability`. This keeps
...
- Reuse the versioned `@relkit/observability` `LogRecord` and 12.4 collector;
...
- Normalize existing Phase 4–10 hook envelopes at the observability package
  ...
  depend on `@relkit/observability`; direct model records cover job, event,
...
- Keep `packages/observability/src/redaction.ts` as the only admission seam
  and reuse `@relkit/contracts` canonical JSON/deep-freeze utilities; no runtime
...
- Reuse `@relkit/contracts` JSON primitive/value types. Shared correlation fields
...
- Keep the focused file-selector tests as the merge-blocking check. The broad
  ...
  discovery after all RELKIT assertions pass; it is not a RELKIT implementation
...
- Treat the exact package-root selector's protected `repos/effect` discovery
  failure as a known non-blocking limitation because every RELKIT assertion is
...
- Keep the `@relkit/testing` public declaration exception narrow: its intentional
...
- Reuse `@relkit/testing`'s isolated fake-model harness and provide a minimal
...
- Keep the matrix in `@relkit/testing` tests so it composes the existing isolated
...
- Preserve declared target error IDs as model-visible safe error codes only
  when they are declared on the target function. Framework `RELKIT_*` codes stay
...
- Keep the harness in `@relkit/testing` and require the caller to supply the
...
- Keep the fake at the local-provider boundary and implement the existing
  `@relkit/agents` `ModelProvider` contract directly; no vendor SDK, fetch
...
- Reuse the existing `onSpanStart`, `onSpanComplete`, `onObservedEdge`, and
  ...
  agents package remains independent of `@relkit/engine`; tool requests forward
...
- Reuse `@relkit/tools` for catalog lookup, allowlist enforcement, argument
  validation, and engine invocation through its structural `ToolEngine` seam;
  do not add an `@relkit/engine` dependency or duplicate tool execution logic.
...
- Represent each agent's executable identity as one normal function node and
  one normal manifest handler keyed by `relkit.agent.<agentId>.invoke`; retain
...
- Use the existing canonical JSON serializer to measure input and output bytes,
  ...
  or credential types into `@relkit/agents`.
...
- Keep the approval module independent of `@relkit/engine`; it reuses the
...
- Keep the runtime package-independent from `@relkit/engine`: a structural
...
- Extend the existing `packages/events/source-export.test.ts` guard rather
  ...
  contracts, and inspector sources; optional `.relkit/generated` and `.relkit/build`
...
- Reuse `@relkit/testing`'s event fake and caller-owned state root so the child
...
- Keep the canonical publish result shape in the shared functions client
  ...
  `EventEnvelope` from `@relkit/events` by deriving it from that result without
...
- Use one reusable `registerJobContractSuite` with a thin deterministic
  `@relkit/testing` adapter. The harness already composes the public job client,
...
- Use one versioned `relkit.jobs.admin` contract family at the shared protocol
...
- Keep the public enqueue/status types in the existing functions client
  contract and re-export them from `@relkit/jobs`; this preserves the existing
...
- Own the parser in `@relkit/providers-local`, matching the normative Phase 8
  ownership of `packages/providers-local/src/jobs/scheduler.ts`; keep the
  public `@relkit/jobs` descriptor package free of runtime dependencies.
...
- Resolve one provider-owned state root per local generation from the explicit
  ...
  `process.cwd()/.relkit/state`; create only the `buckets` and `cache` profile
...
- Keep cache clients in `@relkit/cache` and re-export the same conditional
  `CacheClient` type through `@relkit/functions`; a value contract whose output
...
- Reject traversal, absolute/drive-relative paths, nulls, backslashes, dot
  segments, and reserved `.relkit`/`__relkit` prefixes before any storage lookup;
...
- Keep the public bucket contract and error types in `@relkit/buckets`, with
  `@relkit/functions` re-exporting the client-facing types. The bridge is a
...
- Keep the public app seam structural (`request`/`fetch`) so the in-memory adapter accepts the chosen HTTP framework without adding framework types or dependencies to `@relkit/testing` declarations.
...
- Include ordered middleware/transform IDs and contract, graph, and generator versions as `x-relkit` metadata. Leave compiler artifact writing and content-aware OpenAPI output wiring to checkbox `7.9`.

## Task 7.7 versioned internal endpoints

- Keep the phase-6 surface in `@relkit/runtime-hono` as a thin adapter over `@relkit/contracts` version constants and safe callback stubs; the later `@relkit/inspector-api` package owns the complete query/action protocol.
- Return canonical JSON envelopes with `relkit.inspector` protocol/version fields, bounded list queries, finite cursor-bearing SSE events, and no manifest handlers or provider clients. The default graph response is a JSON-safe snapshot of the existing registration plan.
...
- Persist isolation as the path contract (`mkdtemp` workspace plus `.relkit/state`, deterministic IDs/clock, fresh fakes, bounded close, and explicit failed-root retention) rather than a machine-specific temporary path.
...
- Allocate a fresh temporary workspace with `.relkit/state` for each `TestRuntime`; use `mkdtemp` rather than a fixed directory or port. An explicitly supplied `stateRoot` is caller-owned and is preserved so later restart tests can reuse it deliberately.
...
- Mark rejected invocations and timed-out shutdowns as failures, and accept an explicit `close({ failed: true })` signal for assertion failures that occur outside runtime work. Retain only temporary failed roots when `RELKIT_KEEP_TEST_STATE=1`; successful roots are removed.

## Task 6.9 standalone testing runtime

- Route `invokeFunction` through the engine's existing direct invocation path and keep transport/provider materialization out of this unit. Resolve app test values with `@relkit/config`, while standalone calls receive a copied frozen environment and the engine's frozen default context.
...
- Keep the versioned `relkit.observability.hooks` v1 contract at the engine boundary and emit invocation, span, declared-edge, observed-edge, completion, and release events through the existing invocation hooks.
...
- Reject any repeated function ID before appending a frame, covering direct recursion and multi-function cycles with one iterative membership check. Use the fixed safe `RELKIT_RECURSION_DENIED` policy error and expose only frozen function/cycle metadata.
...
- Include the current `@relkit/testing` public entrypoint in declaration emission so the scan covers the complete public application package cohort, and extend the existing authoring symbol pattern with `Layer`, `Context.Tag`, `Schema.Schema`, `Fiber`, and `Cause` alongside `Effect`.
...
- Keep the trace state in an internal `Context.Reference` carrying invocation ID, parent invocation ID, trace/span IDs, source, correlation ID, and the caller signal. Root and child helpers install that state around Effect spans and reserve invocation annotations under stable `relkit.*` attribute names.
...
- Normalize synchronous throws and Promise rejections at the bridge boundary with the existing failure algebra; keep declared error detection structural so `@relkit/runtime-effect` does not depend on `@relkit/functions`.
...
- Keep failure implementation and telemetry behind `@relkit/runtime-effect`; do not add an application-package dependency or re-export Effect/Cause types. Preserve the intentionally uncommitted checkout, iterator skill, vendor, and normative documents.

## Task 5.3 generation runtime and scope

- Consume the Phase 1 adapter through the explicitly internal `@relkit/config/internal/config` subpath. The public `@relkit/config` root and application package exports remain plain; the packed-export check allows only this named config-internal entry.
...
- Add only `@relkit/contracts` and `@relkit/graph` as direct workspace dependencies required by the service contracts. Export the tags from `@relkit/runtime-effect`'s internal entry only; do not add any application-package re-export or application dependency.
...
- Keep checkbox 5.1 setup-only. Reuse the Phase 1 `effect` pin and add only the supported Bun platform packages needed by `@relkit/runtime-effect`; do not start service, lifecycle, handler, cancellation, clock, tracing, or logging implementation.
...
- Add the fixture's checked-in `relkit.config.ts` and compile a disposable copy through the existing evaluator/normalizer seam. Reuse the fixture runner rather than adding a second compiler pipeline or compiling source in-process.
...
- Make every compilation disposable and self-contained by copying only source/config into a unique temporary root and linking the local `@relkit/*` package roots needed by the fixture imports. Remove the root in `finally` so parallel or repeated runs cannot share application state.
...
- Keep registration planning in `@relkit/graph` as a graph-only projection. It reuses canonicalization and the single graph hash, returns only provider-free registration data, and deep-freezes the result; provider construction and materialization remain later runtime work.
...
- Use evaluator reference instructions for generated imports: normalize module paths through `@relkit/contracts`, sort unique modules before assigning namespace aliases, and address exports by JSON-quoted names. Direct in-memory descriptors remain accepted by normalization tests, but a missing or malformed reference cannot activate an evaluator-backed manifest.
...
- Make `packages/graph/src/hash.ts` the single owner of canonical graph bytes and graph hashes. Reuse `@relkit/contracts` canonical JSON and source-location normalization rather than duplicating either contract in the compiler.
...
- Keep `@relkit/contracts` as the graph package's only dependency. The compiler's existing loose normalization graph remains unchanged for checkbox `4.10` construction work.
...
- Treat middleware and named transforms as compiler-only internal descriptor shapes. Capture their evaluator metadata structurally, project schemas to JSON-safe snapshots or report `RELKIT_SCHEMA_UNAVAILABLE`, and do not add graph model/node behavior owned by checkboxes `4.9` and `4.10`.
...
- Normalize explicit IDs, HTTP methods/paths, profile names, schedule fields, retry jitter, and idempotency keys before local validation. Use deterministic fallback IDs for malformed raw inputs so `RELKIT_ID_INVALID` and sibling diagnostics are emitted rather than allowing diagnostic normalization to throw.
...
- Keep detection inside the existing Bun child and install/restore hooks around each candidate import. A detected operation becomes a structured `RELKIT_EVALUATOR_SIDE_EFFECT` failure; the parent still owns timeout/kill and never falls back to in-process evaluation.
- Allow writes only beneath the request's project-relative generated directory, defaulting to `.relkit/generated`; common Node filesystem mutators and `Bun.write` are guarded lexically. The detector intentionally does not claim to cover file descriptors, symlink/race escapes, native syscalls, or pre-bound CommonJS named imports.
...
- Use the approved candidate indicators as a conservative syntax filter: runtime `@relkit/*` imports, known descriptor factory call names, default exports, descriptor-brand access, and runtime re-exports. Ignore type-only imports/exports so type contracts alone do not activate evaluation.
...
- Declare `typescript` as an `@relkit/compiler` runtime dependency because the packed compiler executes the TypeScript AST API. Extend the existing export smoke only to stage declared external dependencies from available root/workspace-local installations; no new package or runtime abstraction was added.
...
- Keep `relkit.config.ts` tooling-only. The loader allowlists `entry`, `source`, `exclude`, `generatedDirectory`, and `inspector`; it neither imports nor evaluates application modules and rejects unknown/function-valued behavior.
...
- Preserve the exact package-root Gate 2 command and its exit `1`: all ten RelKit tests pass, then Bun discovers unrelated vendored `repos/effect/packages/tools` tests with missing upstream modules. The focused RelKit entrypoint is the applicable passing descriptor evidence; do not broaden discovery, add a workaround, or touch vendor files here.
...
- Run the exact package-root command required by the checkbox even though Bun's unscoped basename discovery reaches `repos/effect/packages/tools`. Preserve the command's exit `1` and missing-upstream-module output as a known repository limitation; use the focused RelKit package entrypoints to account for the ten RelKit tests without touching vendored files.
- Record the convention smoke's five warning-only codes explicitly. All are `RELKIT_CONVENTION_DIRECTORY`, `RELKIT_CONVENTION_SUFFIX`, `RELKIT_CONVENTION_EXPORT`, `RELKIT_CONVENTION_MULTIPLE_KINDS`, and `RELKIT_CONVENTION_ID_STYLE`; none is an error and the branded descriptor remains included.
...
- Use the common public `@relkit/app` re-export surface plus public `@relkit/schema` for the fixture. Do not add a fixture package manifest or lockfile entry yet: Phase 2 requires authoring only, and fixture compilation/package resolution is owned by Phase 3.
...
- Reuse `@relkit/contracts` for descriptor/path validation and `@relkit/diagnostics` for portable warning records. Use the v3 directory/suffix table, default-export/file-kind facts supplied by discovery, and a lower-case dot/kebab recommendation for the advisory ID-style warning.
...
- Export the helper through `@relkit/compiler` and declare only its two existing lower-layer dependencies. Leave discovery, graph construction, fixture authoring, and the durable convention test cohort to their owning checkboxes.
...
- Reuse the shared descriptor brand/freeze contract, Standard Schema v1 validation, and `@relkit/tools`'s `isToolRef`; store only copied `{ ref }` values so agent metadata cannot retain tool targets or executable details.
...
- Scan source files and package manifests for forbidden public names, then inspect the `@relkit/events` runtime export keys directly. The test also exercises negative synthetic inputs so a future public descriptor/type or source suffix fails with a stable finding.
...
- Extend `@relkit/contracts` with the shared descriptor brand/base rather than duplicating the symbol or guards across descriptor packages. Reuse the existing `DescriptorKind`, `Ref`, and `normalizeId` contracts so identity remains explicit and path-independent.
...
- Keep declared-error refs local to `@relkit/functions` with kind `error` because the shared v3 `DescriptorKind`/graph-node union intentionally excludes errors. Defer invocation metadata, `AbortSignal`, env, logger, and clock fields to checkbox `3.4`.

## Task 3.4 public function context

- Add Section 7.6's context contracts to `@relkit/functions` and re-export them through the existing function barrel: invocation metadata uses the six documented source values, `signal` is the platform `AbortSignal`, logger methods are synchronous `void`, and clock methods use `Date` plus `Promise<void>` as shown by the v3 examples.
...
- Reuse `@relkit/contracts` source-location, stable-ID, and canonical-JSON contracts instead of duplicating path or serialization rules. The diagnostics package therefore declares only that existing workspace dependency and keeps its public values plain TypeScript.
...
- Keep checkbox `2.10` test-only: reuse `@relkit/config`'s existing `defineEnv`, `resolveEnv`, and `projectEnv` seams rather than changing the verified 2.9 resolver or private Effect adapter.
...
- Task `2.8` keeps declaration-time environment work dependency-free and side-effect-free. `env.ts` owns the builder DSL, `env-types.ts` owns public type declarations, and `env-json.ts` owns recursive JSON-safe example/value conversion so every implementation file stays under 200 lines; `index.ts` re-exports the documented `@relkit/config` surface.
...
- Use `fix/implement-relkit-typescript-poc-v3` in the normal checkout so progress remains visible. Preserve existing planning-artifact edits and the supplied iterator skill; do not stage or commit.
...
- Keep the root `dev` script as `turbo run dev`, the actual available workspace dispatcher, until the later package/CLI tasks own RelKit supervisor behavior. With no package shells yet, a successful zero-task dispatch is the truthful Phase 0 result; no fake server or placeholder runtime was added.
...
- Task `1.4` uses one uniform public-shell manifest: `@relkit/<name>` at version `0.0.0`, ESM, a root-only `types`/`import` export to `src/index.ts`, and `build`/`check`/`typecheck` scripts backed by the shared TypeScript config. No package dependencies or runtime behavior are introduced.
...
- A root-level package import smoke is deferred until a dependent workspace fixture exists because the root intentionally has no `@relkit/*` dependencies. Package-local self-reference imports pass, and deep `@relkit/*/src/*` imports are rejected by each shell's export map.
- Task `1.5` applies the same three-file shell convention to the 13 foundation/internal packages: `@relkit/<name>` version `0.0.0`, ESM, root-only `types`/`import` exports to `src/index.ts`, shared TypeScript config, and no dependencies or runtime behavior. This keeps internal ownership explicit without implementing task `1.6` or later package APIs.
...
- Task `1.6` reuses the three-file shell convention for `packages/{deploy,deploy-pulumi,cloud-aws,cli,create-relkit}`. The first four package names use `@relkit/<name>`; `create-relkit` remains unscoped because the approved `bunx create-relkit@latest` entry point depends on that published package name.
- Task `1.6` exposes `relkit` from `@relkit/cli` and `create-relkit` from `create-relkit` through manifest `bin` entries pointing at the existing side-effect-free `src/index.ts`. No command implementation, runtime dependency, extra source file, or task `1.7` configuration was added.
...
- Task `1.8` changes every package's root-only export map from source targets to publishable `./dist/index.d.ts` and `./dist/index.js` targets. The two approved bin keys remain unchanged by name and now point at the built `./dist/index.js`; `create-relkit` remains the only unscoped package.
- The export smoke builds and packs `@relkit/app` and `@relkit/compiler`, installs the tarballs in a temporary directory outside the workspace, and uses Node's package resolver because Bun's workspace resolver can accept existing deep source paths. The root exports resolve; `src/index.ts` and `dist/index.js` subpaths fail through the root-only export map. No runtime implementation or extra package dependency was added.
...
- Fixture/template imports use the v3 Section 6.2 public application packages as an allowlist for `@relkit/*`; raw Effect/Hono/Next/Pulumi/AWS SDK packages and all other RelKit implementation packages are rejected. Inspector rules allow its own Next.js dependency but reject application packages, the fixture package, and runtime/provider/deployment implementations.
...
- Task `1.13` records each approved architecture decision in its own sequential `RELKIT-ADR-001` through `RELKIT-ADR-007` file. The ADRs use `Accepted — reviewed Phase 0 baseline` because the v3 documents are already approved and the records document implementation constraints rather than propose new scope.
...
- Use root-anchored, directory-specific `.relkit` patterns with trailing slashes so generated/build/runtime data is ignored without broad patterns that could hide checked-in fixtures, goldens, templates, or OpenSpec/review evidence.
...
- Keep projection dependency-free and behind the existing `relkit.jsonSchema` compatibility hook. The repository has no schema vendor dependency, and adding one would expand the public boundary before a concrete runtime need.
...
- Return the stable `RELKIT_SCHEMA_UNAVAILABLE` code for absent hooks, invalid JSON Schema values, custom refinements, transforms, and non-deterministic default factories. Compiler/source-location diagnostics remain the compiler owner's responsibility.
...
- Use one third-party fixture with a deterministic `relkit.jsonSchema` hook and a separate compatible fixture without that hook. This proves both Standard Schema validation compatibility and the structured `RELKIT_SCHEMA_UNAVAILABLE` result without adding a schema dependency.
...
- Use `@relkit/schema` as the documented schema entry point, with `z`, Standard Schema validation, structured issues, and deterministic JSON Schema projection. Do not document internal schema implementations.
- Keep `@relkit/config` declaration and resolution visibly separate: `defineEnv` builds immutable metadata without runtime reads, `projectEnv` is safe metadata, and `resolveEnv` receives an explicit source inside startup code.
...
- Keep the concrete RelKit template dependency version at `0.0.0`: it matches
...
- Run the packed `create-relkit` API and packed `relkit create` with one option
