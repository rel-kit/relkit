...
Event authoring SHALL expose `defineEvent`, selector helpers, and `onEvent`; `onEvent` SHALL create an `event-trigger` descriptor targeting a function, and RelKit SHALL NOT expose `defineSubscription`, a subscription graph node, or a `*.subscription.ts` convention.
...
- **THEN** compilation includes it and emits `RELKIT_CONVENTION_DIRECTORY` or `RELKIT_CONVENTION_SUFFIX` without a non-zero exit solely for that warning
...
- **THEN** RelKit models only the function and its explicitly declared RelKit dependencies, not the library's internal resources or behavior
