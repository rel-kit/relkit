...
```text
docker build --no-cache --build-arg SOURCE_DATE_EPOCH=0 --tag <image-a> --iidfile <id-a> .relkit/build
docker build --no-cache --build-arg SOURCE_DATE_EPOCH=0 --tag <image-b> --iidfile <id-b> .relkit/build
```
...
| Contents scan               | `/app` contains only `application.graph.json`, `manifest.json`, `openapi.json`, and `server/index.js`; `.env`, `.relkit/state`, and `.relkit/observability` are absent                    |
| Liveness/readiness ordering | With `RELKIT_PROVIDER_READY_DELAY_MS=1000`, liveness returned `200`, first readiness returned `503` with `providerReady=false`, then readiness returned `200` with `providerReady=true` |
