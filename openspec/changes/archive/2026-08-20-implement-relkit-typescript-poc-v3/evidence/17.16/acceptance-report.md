...
reported protocol `relkit.performance` version `1`. Run 2 at
...
`/tmp/relkit-*` temporary state roots remained, and Git showed no performance
or test-source drift. The ignored workspace `.relkit` state observed during the
