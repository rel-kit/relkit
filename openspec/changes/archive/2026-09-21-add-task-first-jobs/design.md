...
The default quickstart is `docker(inngest())`, with both durable-task and native-scheduling acceptance required. `docker(effectMq())` supplies the smaller queue/retryable alternative, using a recipe-owned PostgreSQL store by default; a Redis variant is certified separately. `docker(trigger())` is a separate self-hosted mode, not a claim of managed-cloud checkpoint parity. All three recipes remain in implementation scope; this change's executable gate requires the Inngest and effect-mq local paths, while Trigger and Pulumi/AWS/managed-cloud evidence are explicitly deferred and must remain not-tested. No recipe may be advertised merely because a factory exists.
...
```ts
type JobClientOperation = "trigger" | "get" | "list" | "watch" | "cancel" | "retry" | "stream";
...
A task can invoke a declared agent through `ctx.agents` and pass cancellation, trace, and explicit thread/run correlation. Existing agent state providers and native graph checkpointers remain authoritative for agent continuation. This proposal removes only the earlier _new job-state requirement_, not existing agent-state configuration needed by agent features.
...
Core release gate: task/job authoring + default binding + account-free durable Docker + account-free native scheduling path + typed client + Inspector + migration/docs + all corresponding security/restart tests. The current change gate certifies the local Inngest and effect-mq subsets; Trigger native/Docker evidence and Pulumi, AWS, and managed-cloud evidence are explicitly deferred and must remain not-tested. Provider packages are still cataloged separately, and no deferred mode may be promoted to a support claim.
...
CamelCase job names, dot-access generated examples, name/ID migration safety, the exact three-provider implementation scope, docs, generated types, compatibility migration, scaffold smoke, package exports/boundaries, local service regression, security tests, observer cleanup, and old-build recovery are mandatory. This change's certification audit covers Inngest and effect-mq; Trigger and Pulumi/AWS/managed-cloud evidence remain explicitly deferred and not-tested. No passing checklist item can be satisfied solely by this change; attach test evidence during implementation.
