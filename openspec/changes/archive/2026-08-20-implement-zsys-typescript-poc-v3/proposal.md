...
- Add the `zsys` CLI, atomic `create-zsys` scaffolder, generated project templates/tests, provider-neutral deployment planning, Pulumi Automation API commands, and the first AWS runtime/resource mapping.
...
- `cli-scaffolding`: ZSys CLI commands, environment/doctor workflows, atomic `create-zsys` generation, templates, and packed-project acceptance.
...
- **Public APIs**: introduces `@zsys/app`, `@zsys/schema`, `@zsys/config`, descriptor capability packages, `@zsys/testing`, the `zsys` binary, and `create-zsys`; applications author ordinary sync/async TypeScript handlers and Standard Schema-compatible contracts.
...
- **Generated/runtime data**: adds deterministic `.zsys/generated` and `.zsys/build` outputs plus ignored local `.zsys/state` and `.zsys/observability` data; graph artifacts never contain resolved secrets or executable closures.
