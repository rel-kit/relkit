...
**Alternative considered:** implement a new counter/middleware package. Rejected because the dependency already provides the HTTP algorithm/header behavior and ZSYS only needs the storage/graph boundary.

### 5. Event callbacks are authoring sugar over generated functions

`@zsys/events` will declare an augmentable `EventRegistry` and type `onEvent` against its string keys. Compiler discovery first collects event descriptors, writes a deterministic declaration into `.zsys/generated`, then performs normal evaluation/normalization. Runtime validation still checks the named event against the actual descriptor set, preventing stale declarations from activating.
...
Add `defineConfig` at `@zsys/app/config` and reduce accepted keys to nested `server` and `inspector` values. Internally, config loading still produces the compiler/supervisor options expected today, but fixed constants provide `src/app.ts`, `src/**/*.ts`, standard exclusions, `.zsys/generated`, and `.zsys/build`. Legacy path keys are rejected rather than ignored.
