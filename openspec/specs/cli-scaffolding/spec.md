...
The `zsys` CLI SHALL provide documented help, deterministic exit codes, human and `--json` output, and commands for development, checking, building, starting, graph print/check/diff, environment operations, doctor, project creation, and deployment delegation.
...
`bunx create-zsys@latest <name>` and `zsys create <name>` SHALL generate a TypeScript/Bun project using internal Hono, `@zsys/schema`, built-in local/test providers, Pulumi/AWS defaults, the minimal hello function/route/tests, Bun package management, and Git initialization when available unless flags override supported choices.
