...
The repository SHALL provide working format, lint, implementation-file-size, documentation, and public-API quality checks, and SHALL keep `AGENTS.md` synchronized with the actual topology, ports, scripts, test availability, vendored-reference rules, and structural workflow.

#### Scenario: Phase 0 replaces the starter topology

- **WHEN** starter applications, packages, commands, or verification behavior are replaced
- **THEN** the same phase updates `AGENTS.md`, contributor-facing commands, and automated checks so none describes the removed starter as current truth

#### Scenario: Implementation exceeds a repository quality limit

- **WHEN** an implementation file exceeds 200 lines, public API documentation is absent, formatting/lint fails, or behavior changes without its nearest README/spec update
- **THEN** verification or the phase review rejects the change with the affected path and rule
