        ...
        bindings: [
          {
            ...
            list,
            get: async () => snapshot("run-1", "running"),
          },
        ],
      },
    },
...
function snapshot(runId: string, status: RunSnapshot["status"]): RunSnapshot {
  return {
    ...
    resultAvailability: "not-selected",
  } as RunSnapshot;
}
...
describe("Inspector job name filters", () => {
  test("resolves names before bounded native queries", async () => {
    ...
    const app = makeApp(async (value) => {
      query = value;
      return { items: [snapshot("run-1", "running")], hasMore: false, availability: [] };
    });
    const response = await app.request(
      "http://fixture/_relkit/v1/jobs/runs?job=emails&status=running&acceptedFrom=2026-09-16T00:00:00.000Z&limit=25",
    );
    expect(response.status).toBe(200);
    expect(query).toMatchObject({
      jobId: "job.emails",
      status: ["running"],
      acceptedFrom: "2026-09-16T00:00:00.000Z",
      limit: 25,
    });
  });
  ...
  test("rejects a cursor after a filter change", async () => {
    const app = makeApp(async () => ({
      items: [snapshot("run-1", "running")],
      hasMore: true,
      nextCursor: "native-1",
      availability: [],
    }));
    const first = await app.request(
      "http://fixture/_relkit/v1/jobs/runs?nativeQuery=provider-one&limit=1",
    );
    const cursor = (await first.json()).nextCursor;
    const changed = await app.request(
      `http://fixture/_relkit/v1/jobs/runs?nativeQuery=provider-two&cursor=${cursor}`,
    );
