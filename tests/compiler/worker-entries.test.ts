...
describe("immutable job worker entries", () => {
  test("writes deterministic worker and routing files without rewriting unchanged bytes", async () => {
    ...
    try {
      const first = await writeJobWorkerEntries([entry("orders.send")], { buildDirectory: directory });
      ...
      expect(await readFile(join(directory, "jobs", "sha256:build-1", "sha256:service-1", "routing.manifest.json"), "utf8")).toContain('"jobId":"orders.send"');

      const second = await writeJobWorkerEntries([entry("orders.send")], { buildDirectory: directory });
  ...
  test("preflights every path so an immutable conflict cannot partially activate a build", async () => {
    const directory = await mkdtemp(join(tmpdir(), "relkit-job-workers-"));
    try {
      await writeJobWorkerEntries([entry("orders.first", "sha256:build-1")], { buildDirectory: directory });
      await expect(
        writeJobWorkerEntries(
          [
      ...
      await expect(readdir(join(directory, "jobs", "sha256:build-2"))).rejects.toMatchObject({ code: "ENOENT" });
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
