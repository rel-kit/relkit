...
describe("durable task replay advisories", () => {
  test("warns about unstable wait identity and work before a wait without blocking activation", () => {
    ...
    const advisories = result.diagnostics.filter(
      ({ code }) => code === "RELKIT_JOB_REPLAY_ADVISORY",
    );
    expect(advisories).toHaveLength(2);
    expect(
      advisories.every(({ severity, file }) => severity === "warning" && file === source.file),
    ).toBe(true);
    expect(
      advisories.some(
        ({ suggestion }) => typeof suggestion === "string" && suggestion.includes("accepted-input"),
      ),
    ).toBe(true);
