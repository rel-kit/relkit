...
import { openNativeStore, persistNativeControl, persistNativeRun } from "./native-adapter-storage.js";
...
export function createLocalNativeJobProvider(root: string, profile: string): JobsAdapterRuntime {
  ...
  const adapter: JobsAdapterRuntime = {
    ...
    submit: (request, context) => serialize(async () => {
      ensureOpen();
      ...
      if (existing !== undefined) return { ...handle(state.runs.get(existing)!), duplicate: true };
    ...
    get: async (locator, context) => {
      ...
      if (run === undefined || !sameNamespace(run, context)) throw new Error("Native run was not found");
      return snapshot(run);
    },
    ...
    cancel: (request, context) => serialize(async () => {
      ensureOpen();
      const store = await readyStore();
      ...
      await persistNativeControl(store, "cancel", controlKey("cancel", request.runId, request.operationId, context), receipt);
      return receipt;
    }),
    retry: (request, context) => serialize(async () => {
      ensureOpen();
      const store = await readyStore();
      ...
      await persistNativeControl(store, "retry", controlKey("retry", request.runId, request.operationId, context), receipt);
      return receipt;
    }),
    worker: {
      next: (context) => serialize(async () => {
        const store = await readyStore();
        const work = await nextRun(state, context);
        if (work !== undefined) await persistNativeRun(store, state.runs.get(work.envelope.runId)!);
        return work;
      }),
      complete: (runId, output, context) => serialize(async () => {
        const store = await readyStore();
        await completeRun(state, runId, output, context);
        const run = state.runs.get(runId);
        if (run !== undefined) await persistNativeRun(store, run);
      }),
      fail: (runId, error, context) => serialize(async () => {
        const store = await readyStore();
        await failRun(state, runId, error, context);
        const run = state.runs.get(runId);
        if (run !== undefined) await persistNativeRun(store, run);
      }),
      suspend: (runId, value, context) => serialize(async () => {
        const store = await readyStore();
  ...
  async function* observeAfterReady(request: Parameters<NonNullable<JobsAdapterRuntime["observe"]>>[0], context: Parameters<NonNullable<JobsAdapterRuntime["observe"]>>[1]) {
    await readyStore();
    yield* observeRun(state, request, context);
  }

  async function readyStore() {
    storePromise ??= openNativeStore(join(root, "jobs", profile, "native"), state).then(async (store) => {
  ...
  function assertRequestScope(request: { readonly scope?: string }, context: { readonly scope: string }): void {
