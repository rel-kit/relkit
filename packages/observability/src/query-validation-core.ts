...
function validate(value: ObservabilityQueryRequest, maximum: number): NormalizedQuery {
...
function matches(record: ObservabilityRecord, query: NormalizedQuery): boolean {
...
function inTimeRange(timestamp: string, query: NormalizedQuery): boolean {
...
function response<T>(items: readonly T[], nextCursor?: string): ObservabilityQueryPage<T> {
...
/** Internal pure query rules executed by the observed Effect operations. */
export const queryValidationCore = { validate, matches, inTimeRange, response, positive };
