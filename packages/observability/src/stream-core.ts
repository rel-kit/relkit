...
const admitRecord = redactionCore.redactRecord;
const { createStreamSubscriber } = streamSubscriberCore;
const { assertType, bounded, invalid, positive, resolveCursor, validateCursor } = streamUtilsCore;
/**
 * Creates a bounded in-memory stream with replay and consumers.
 *
 * @param options - Retention, subscriber, overflow, and redaction settings.
 * @returns A stream that owns subscriber queues until close.
 * @throws {ObservabilityStreamError} If a configured bound is invalid.
 * @example
 * const stream = createObservabilityStream({ maxEvents: 100 });
 * try { stream.replay(); } finally { stream.close(); }
 */
function createObservabilityStream(options: ObservabilityStreamOptions = {}): ObservabilityStream {
  ...
  const subscribe = (input: NonNullable<Parameters<ObservabilityStream["subscribe"]>[0]> = {}) => {
    if (closed)
      throw new ObservabilityStreamError("RELKIT_OBSERVABILITY_STREAM_CLOSED", "Stream is closed");
  ...
  function admitModel(record: ObservabilityRecord): JsonValue | undefined {
    if (options.collector === undefined)
      return recordAdmissionCore.admitRedacted(
        redactionCore.redactRecord(record, options.redaction),
      ) as JsonValue | undefined;
    const accepted = options.collector.collect(record);
    return accepted === undefined ? undefined : (accepted as unknown as JsonValue);
  }
}
/** Internal stream state owned by an Effect Scope or compatibility adapter. */
export const streamCore = { createObservabilityStream };
