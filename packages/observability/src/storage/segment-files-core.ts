...
async function ensureSegmentRoot(requestedRoot?: string): Promise<string> {
...
async function repairSegments(root: string, policy?: RedactionPolicy): Promise<void> {
  await Promise.all(
...
async function listSegments(directory: string): Promise<SegmentFile[]> {
...
async function appendLine(handle: FileHandle, line: string): Promise<void> {
...
function segmentDirectoryFor(signal: unknown): SegmentDirectory | undefined {
  ...
  for (const line of contents.split("\n")) {
    ...
    try {
      const safe = isRecord(value)
        ? recordAdmissionCore.admitRedacted(redactionCore.redactRecord(value, policy))
        : undefined;
