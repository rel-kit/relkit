...
import type { RequestExecutionDetail, SpanNode } from "./execution-assembly.types.js";
 ...
 *
 * @param records - Records that may participate in the request execution.
 * @param requestId - Request to resolve.
 * @returns Immutable detail, or undefined when the request is absent.
 * @example
 * const detail = assembleRequestExecution(records, "request-1");
 */
function assembleRequestExecution(
...
/**
 * Keeps the latest lifecycle revision for each trace and span identity.
 *
 * @param spans - Span lifecycle records in any order.
 * @returns Current spans ordered by start time.
 * @example
 * const current = coalesceSpans(spanRecords);
 */
function coalesceSpans(spans: readonly SpanRecord[]): SpanRecord[] {
 ...
 * @returns The current spans and optional latest trace.
 * @example
 * const current = currentTrace(spans, traces);
 */
function currentTrace(spans: readonly SpanRecord[], traces: readonly TraceRecord[] = []) {
  const currentSpans = coalesceSpans(spans);
  return { spans: Object.freeze(currentSpans), trace: traces.at(-1) };
}
/** Internal pure assembly rules executed by the observed Effect operations. */
export const executionAssemblyCore = { assembleRequestExecution, coalesceSpans, currentTrace };
