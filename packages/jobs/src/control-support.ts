...
export function requireMethod(runtime: JobsRuntime, capability: string, method: keyof JobsRuntime["adapter"]): void {
  if (typeof runtime.adapter[method] !== "function") throw new JobsCapabilityError(capability);
  if (runtime.capabilities.features[capability] === false) throw new JobsCapabilityError(capability);
  if (runtime.capabilities.capabilities?.[capability] !== undefined || runtime.capabilities.features[capability] === true) {
    assertJobsCapability(runtime.capabilities, capability);
  }
}

export function requireOperationId(value: string): void {
  if (typeof value !== "string" || value.trim() === "" || new TextEncoder().encode(value).byteLength > 256) {
...
export function normalizeCancellationReceipt(
  ...
  if (!isRecord(value) || value.runId !== runId || value.operationId !== operationId || !isCancellationOutcome(value.outcome)) {
  ...
  if (value.run !== undefined && !isRecord(value.run)) throw new TypeError("Native cancel receipt run is invalid");
...
export function normalizeRetryReceipt(value: unknown, originalRunId: string): RunRetryReceipt {
  ...
  if (value.runId === originalRunId) throw new TypeError("Native retry receipt did not create a new run");
  return Object.freeze({
    accepted: true,
...
export function unknownKey(value: unknown): string | undefined {
  return isRecord(value) && typeof value.idempotencyKey === "string" ? value.idempotencyKey : undefined;
}
...
export function unknownRecovery(value: unknown): JobUnknownOutcome["recovery"] | undefined {
  ...
  if (recovery.action !== "retry-with-same-key" && recovery.action !== "inspect-native" && recovery.action !== "unavailable") {
    return undefined;
  }
  return {
...
export function observerTimeout(runtime: JobsRuntime, authored?: DurationInput): number {
  const timeoutMs = authored === undefined ? runtime.capabilities.limits?.readTimeoutMs ?? 10_000 : durationToMillis(authored);
...
function nextWithTimeout<T>(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = (): void => finish(() => reject(signal.reason ?? new Error("Observation aborted")));
    const timer = setTimeout(() => finish(() => reject(new JobObservationTimeoutError(timeoutMs))), timeoutMs);
    if (signal.aborted) {
      onAbort();
      return;
    }
