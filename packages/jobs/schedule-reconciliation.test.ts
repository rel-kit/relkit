...
test("reconciles owned schedules after worker publication and preserves operator schedules", async () => {
  ...
  const result = await reconcileNativeSchedules({
    ...
    jobId: "job",
    buildId: "build",
    workerReady: async () => { calls.push("worker-ready"); },
  });
  ...
  expect((writes[0] as { readonly metadata: { readonly canonicalInput: string } }).metadata.canonicalInput).toBe("{\"value\":1}");
});
...
test("rejects an ownership collision instead of changing operator state", async () => {
  ...
  await expect(reconcileNativeSchedules({
    native,
...
test("keeps an owned operator pause while updating input/build and gives each tick a fresh identity", async () => {
  const paused: string[] = [];
  const native: NativeScheduleOperations = {
    list: async () => ({ schedules: [{ id: "paused", state: "paused", metadata: { owner: scheduleOwner({ context, jobId: "job" }) } }] }),
    get: async () => ({ outcome: "unavailable" }),
    upsert: async () => ({ outcome: "updated" }),
    pause: async (id) => { paused.push(id); return { outcome: "paused" }; },
    resume: async () => ({ outcome: "resumed" }),
    delete: async () => ({ outcome: "deleted" }),
  };
  ...
  expect(scheduleOccurrenceIdentity("paused", "2026-09-15T00:00:00.000Z"))
    .not.toBe(scheduleOccurrenceIdentity("paused", "2026-09-15T00:05:00.000Z"));
  expect(scheduleOccurrenceIdentity("paused", "2026-09-15T00:00:00.000Z"))
    .toBe(scheduleOccurrenceIdentity("paused", "2026-09-15T00:00:00.000Z"));
});
