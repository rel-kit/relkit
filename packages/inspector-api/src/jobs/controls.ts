...
export async function controlJobRun(
  ...
  const service = new URL(request.url).searchParams.get("service") ?? (typeof body.service === "string" ? body.service : undefined);
  ...
  const operationId = request.headers.get("x-relkit-operation-id") ?? (typeof body.operationId === "string" ? body.operationId : crypto.randomUUID());
  if (operationId.length === 0 || operationId.length > 256) throw new InspectorJobsError("RELKIT_INSPECTOR_JOBS_OPERATION_INVALID", 400);
  const context = operationContext(generation, binding, "control", request);
  try {
    if (action === "cancel") {
      if (binding.cancel === undefined || binding.capabilities?.features?.cancel === false) throw new InspectorJobsError("RELKIT_INSPECTOR_JOBS_OPERATION_UNSUPPORTED", 501);
      const reason = typeof body.reason === "string" ? body.reason : undefined;
      return controlResponse(generation, binding, await binding.cancel(runId, operationId, reason, context));
    }
    if (binding.retry === undefined || binding.capabilities?.features?.retry === false) throw new InspectorJobsError("RELKIT_INSPECTOR_JOBS_OPERATION_UNSUPPORTED", 501);
...
function controlResponse(generation: ResolvedActiveGeneration, binding: InspectorJobsBinding, receipt: unknown): JsonValue {
...
async function findBinding(
  ...
  for (const binding of bindings) {
    try { await binding.get(runId, operationContext(generation, binding, "read", request)); return binding; } catch {}
  }
...
async function readBody(request: Request): Promise<Record<string, JsonValue>> {
  try {
    ...
    if (value !== null && typeof value === "object" && !Array.isArray(value)) return value as Record<string, JsonValue>;
  } catch {}
  return {};
}
