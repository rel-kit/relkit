...
export function installJobsEndpoints(app: Hono, guard: InspectorGuard): void {
  ...
  app.get(
    ...
    guard(async (context, generation) => {
      ...
      return json(await listJobDefinitions(active, context.req.raw, active.jobs));
    }),
  );
  app.get(
    `${base}/definitions/:id`,
    guard(async (context, generation) => {
      const active = required(generation);
      await authorizeJobs(active, context.req.raw, "read");
      return json(await getJobDefinition(active, requiredParam(context, "id"), active.jobs));
    }),
  );
  app.get(
    `${base}/tasks/:id`,
    guard(async (context, generation) => {
      const active = required(generation);
      await authorizeJobs(active, context.req.raw, "read");
      return json(getTaskDefinition(active, requiredParam(context, "id")));
    }),
  );
  app.get(
    `${base}/runs`,
    guard(async (context, generation) =>
      json(await listJobRuns(required(generation), context.req.raw)),
    ),
  );
  app.get(
    `${base}/runs/:id`,
    guard(async (context, generation) =>
      json(await getJobRun(required(generation), context.req.raw, requiredParam(context, "id"))),
    ),
  );
  app.post(
    `${base}/runs/:id/cancel`,
  ...
  app.get(
    `${base}/schedules`,
    guard(async (context, generation) =>
      json(await listSchedules(required(generation), context.req.raw)),
    ),
  );
  app.get(
    `${base}/schedules/:id`,
    guard(async (context, generation) =>
      json(await getSchedule(required(generation), context.req.raw, requiredParam(context, "id"))),
    ),
  );
  app.post(
    `${base}/schedules`,
    guard(async (context, generation) =>
      json(await scheduleAction(required(generation), context.req.raw, "upsert")),
    ),
  );
  app.post(
  ...
  app.get(
    `${base}/services`,
    guard(async (context, generation) =>
      json(await listJobServices(required(generation), context.req.raw)),
    ),
  );
  app.get(
    `${base}/services/:id`,
    guard(async (context, generation) =>
      json(
        await getJobService(required(generation), context.req.raw, requiredParam(context, "id")),
      ),
    ),
  );
}
