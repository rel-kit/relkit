...
import { isStreamOutput } from "./stream-runtime.js";
...
/** Tagged failure from standalone invocation orchestration.
 * @example Effect.catchTag(invokeStandaloneEffect(request, options, dispatcher, false), "StandaloneInvocationFailure", () => Effect.void);
 */
export class StandaloneInvocationFailure extends Data.TaggedError("StandaloneInvocationFailure")<{
  readonly cause: unknown;
  readonly message: string;
}> {}

/** Runs a standalone target through ordered policy, validation, lifecycle, and release stages.
 * @param request - Target and input.
 * @param options - Merged dispatch controls.
 * @param dispatcher - Dispatcher for nested invocation scope.
 * @param streamLifecycle - Whether output completion is deferred to stream consumption.
