...
export function createLocalStructuredLoggerEffect(
  ...
  return observeInvocation("logger.create", Effect.sync(() => {
    const entries: StructuredLogRecord[] = [];
    const writeEffect: LocalEffectLogger["writeEffect"] = (level, message, fields) =>
      observeInvocation("logger.write", Effect.flatMap(currentExecutionContextEffect(), (active) =>
        Effect.sync(() => {
          entries.push(Object.freeze({
            ...
            ...(active?.originRequestId === undefined ? {} : { originRequestId: active.originRequestId }),
    ...
    ): void => { runInvocationSync(writeEffect(level, message, fields)); };
    const recordsEffect = (): Effect.Effect<readonly StructuredLogRecord[]> =>
      observeInvocation("logger.records", Effect.sync(() => Object.freeze([...entries])));
    return Object.freeze({
      trace: (message: string, fields?: Readonly<Record<string, unknown>>) => write("trace", message, fields),
      debug: (message: string, fields?: Readonly<Record<string, unknown>>) => write("debug", message, fields),
      info: (message: string, fields?: Readonly<Record<string, unknown>>) => write("info", message, fields),
      warn: (message: string, fields?: Readonly<Record<string, unknown>>) => write("warn", message, fields),
      error: (message: string, fields?: Readonly<Record<string, unknown>>) => write("error", message, fields),
      get records(): readonly StructuredLogRecord[] { return runInvocationSync(recordsEffect()); },
      writeEffect,
      recordsEffect,
    });
  }));
 ...
 */
export function createLocalStructuredLogger(record: InvocationRecord, time: PublicClock): LocalEffectLogger {
