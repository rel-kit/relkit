...
describe("Effect descriptor identity", () => {
  test("uses an isolated Layer for canonical and unbound identities", () => {
    ...
    const descriptor = {};
    expect(Effect.runSync(Effect.provide(createUnboundIdentityEffect(), layer)))
      .toBe("unbound.1");
    expect(Effect.runSync(Effect.provide(resolveDescriptorIdentityEffect(descriptor), layer)))
      .toMatchObject({ id: "unbound.2", canonical: false, key: descriptor });
    expect(Effect.runSync(Effect.provide(getDescriptorIdentityEffect(descriptor), layer)))
      .toBe("unbound.2");
    expect(Effect.runSync(Effect.provide(isDescriptorIdentityBoundEffect(descriptor), layer)))
      .toBe(false);
    expect(Effect.runSync(Effect.provide(bindDescriptorIdentityEffect(descriptor, "orders.find"), layer)))
      .toBe(descriptor);
    expect(Effect.runSync(Effect.provide(isDescriptorIdentityBoundEffect(descriptor), layer)))
      .toBe(true);
  ...
  test("tags conflicting bindings while preserving the public error", () => {
    ...
    expect(() => bindDescriptorIdentity(descriptor, "orders.create"))
  ...
  test("binds service members atomically and resolves their owner", () => {
    ...
    expect(Effect.runSync(Effect.provide(
      bindDescriptorServiceMembersEffect([member], service), layer,
    ))).toEqual([member]);
    expect(Effect.runSync(Effect.provide(getDescriptorServiceIdentityEffect(member), layer)))
      .toBe("orders.service");
    bindDescriptorServiceMembers([member], service);
    expect(getDescriptorServiceIdentity(member)).toBe("orders.service");
    expect(() => bindDescriptorServiceMembers([member], { id: "another.service" }))
      .toThrow("another service");
  });
});
