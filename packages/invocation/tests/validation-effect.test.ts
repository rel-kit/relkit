...
describe("Effect validation boundaries", () => {
  ...
  test("returns parsed values and tags validation failures", async () => {
    ...
    expect(await Effect.runPromise(validatedEffect(schema, { count: 2 }, "input")))
      .toEqual({ count: 2 });
  ...
  test("checks declared application error data", async () => {
    const error = applicationFailure({ id: "errors.duplicate", message: "Duplicate", data: { key: 1 } });
  ...
  test("reports policy failures through tags and keeps TypeError adapters", () => {
    ...
    const source = Effect.runSync(Effect.catchTag(
      assertSourceEffect("other"), "InvocationPolicyFailure", (error) => Effect.succeed(error),
    ));
    ...
    const mode = Effect.runSync(Effect.catchTag(
      assertInvocationModeEffect(target, "direct"), "InvocationPolicyFailure",
      (error) => Effect.succeed(error),
    ));
  ...
  test("substitutes the ID generator through a Layer", () => {
    ...
    expect(Effect.runSync(Effect.provide(nextInvocationIdEffect("invocation"), layer)))
      .toBe("fixed-id");
  });
});
