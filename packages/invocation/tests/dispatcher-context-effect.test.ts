...
describe("standalone context Effect", () => {
  test("installs configured maps and filters published event clients", async () => {
    ...
    const context = await Effect.runPromise(makeStandaloneContextEffect({
      record, signal, env: { mode: "test" }, time, publishes: ["events.posted"],
      clients: {
  ...
  test("tags missing clients and keeps the proxy error", () => {
    const failure = Effect.runSync(Effect.catchTag(
      ...
      "ManagedDependencyFailure", (error) => Effect.succeed(error),
    ));
  ...
  test("preserves a custom context factory rejection", async () => {
    ...
    const options = {
      factory: async () => { throw cause; },
      record, signal: new AbortController().signal, env: {}, time, publishes: [],
    };
