...
describe("root span and snapshots", () => {
  ...
  test("supports a telemetry Layer for span operations", () => {
    ...
    const root = Effect.runSync(Effect.provide(startRootSpanEffect(runtime, "root", "server"), layer));
    Effect.runSync(Effect.provide(completeSpanEffect(root), layer));
    Effect.runSync(Effect.provide(spanSnapshotEffect({ type: "completed", span: root, revision: root.revision }), layer));
  ...
  test("serializes child metadata, captures, links, events, and errors", () => {
    ...
    const child = runtime.start({
      name: "child", parent: Option.some(parent), annotations: Context.empty(),
      links: [], startTime: 1_000_000n, kind: "internal", root: false, sampled: true,
    });
    ...
    const linked = Tracer.externalSpan({ traceId: createTraceId(), spanId: createSpanId(), sampled: true });
