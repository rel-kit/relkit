...
import {
  ...
  tagsFor,
  validatePort,
  type ZsysComponentArgs,
} from "../common.js";

export interface ZsysNetworkArgs extends ZsysComponentArgs {
...
export class ZsysNetwork extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    args: ZsysNetworkArgs = {},
    opts: pulumi.ComponentResourceOptions = {},
  ) {
    const componentName = resourceName(name, "network", args);
    super("zsys:cloud-aws:ZsysNetwork", componentName, {}, opts);
    ...
    this.loadBalancerSecurityGroup = new aws.ec2.SecurityGroup(
      `${componentName}-alb-sg`,
      {
        ...
        description: "ZSys public application load balancer",
        vpcId: this.vpcId,
    ...
    this.serviceSecurityGroup = new aws.ec2.SecurityGroup(
      `${componentName}-service-sg`,
      {
        ...
        description: "ZSys private application service",
        vpcId: this.vpcId,
        ingress: [
          {
