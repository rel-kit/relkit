import { assertJsonValue, canonicalJson, normalizeId, type JsonValue } from "@zsys/contracts";
import type { ZsysJobQueuesArgs, ZsysJobQueueDefinition, ZsysScheduleDefinition } from "./types.js";
...
export interface NormalizedJobQueue {
  readonly id: string;
  readonly retry: ZsysJobQueueDefinition["retry"];
...
export function normalizeJobs(args: ZsysJobQueuesArgs): NormalizedJobQueue[] {
...
export function normalizeSchedules(
  schedules: readonly ZsysScheduleDefinition[] | undefined,
...
export function normalizeSchedulerRetry(
  policy: ZsysJobQueuesArgs["schedulerRetryPolicy"],
...
function validateRetry(policy: ZsysJobQueueDefinition["retry"], id: string): void {
