...
import { childResourceName } from "../RelkitEventBus/names.js";
import { normalizeCaches } from "./validation.js";
import type { RelkitCacheEndpoint, RelkitCacheResource, RelkitCachesArgs } from "./types.js";

export * from "./client.js";
export * from "./types.js";

/** Maps logical caches to ElastiCache Valkey. */
export class RelkitCaches extends pulumi.ComponentResource {
  readonly caches: readonly RelkitCacheResource[];
  readonly cacheResources: readonly RelkitCacheResource[];
  readonly tags: pulumi.Output<Record<string, string>>;

  constructor(name: string, args: RelkitCachesArgs, opts: pulumi.ComponentResourceOptions = {}) {
    const definitions = normalizeCaches(args);
    const componentName = resourceName(name, "caches", args, 64);
    super("relkit:cloud-aws:RelkitCaches", componentName, {}, opts);
    ...
    this.caches = definitions.map((definition) => {
      ...
      const endpoint = cache.endpoints.apply((endpoints): RelkitCacheEndpoint => {
