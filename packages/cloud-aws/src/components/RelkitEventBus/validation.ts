...
import type {
  RelkitEventBridgeRetryPolicy,
  RelkitEventBusArgs,
  RelkitEventDefinition,
  RelkitEventRetryPolicy,
...
export function normalizeEvents(events: readonly RelkitEventDefinition[]): NormalizedEvent[] {
...
export function normalizeEventBridgeRetry(
  policy: RelkitEventBusArgs["eventBridgeRetryPolicy"],
...
export function normalizeSource(source: string | undefined): string {
  const value = source?.trim() ?? "relkit";
