...
export const landingStack = [
  ...
  { name: "Effect", icon: siEffect },
  { name: "Hono", icon: siHono },
  { name: "oRPC", icon: undefined },
  { name: "Pulumi", icon: siPulumi },
] as const;

export const landingCommunityStack = [
  ...landingStack,
...
export function StackIcon({
  ...
  readonly icon: SimpleIcon | undefined;
}) {
  if (name === "AWS") {
    return <Cloud className="landing-stack-icon" role="img" aria-label={name} />;
  }
  if (icon !== undefined) {
    return (
      <svg className="landing-stack-icon" viewBox="0 0 24 24" role="img" aria-label={name}>
        <path d={icon.path} fill="currentColor" />
      </svg>
    );
  }
  return (
    <svg className="landing-stack-icon" viewBox="0 0 256 256" role="img" aria-label="oRPC">
      <path
        fill="currentColor"
