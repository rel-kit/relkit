    ...
    <>
      ...
      <main className="route-page">
        <header className="page-heading">
          <div>
            <p className="eyebrow">JOBS / SERVICES</p>
            <h1>Services</h1>
            <p className="lede">
              Configured jobs providers, capabilities, generations, and health.
            </p>
          </div>
        </header>
        {state === "loading" && (
        ...
        {state === "error" && (
          <p className="panel route-state" role="alert">
            Jobs services are unavailable.
          </p>
        )}
        {state === "ready" && <ServiceTable items={items} />}
      </main>
    </>
  );
}
...
function ServiceTable({ items }: { readonly items: readonly InspectorObject[] }) {
  return items.length === 0 ? (
    <p className="panel route-state">No jobs services are configured.</p>
  ) : (
    <div className="panel overflow-x-auto">
      <table className="resource-table">
        <caption className="sr-only">Jobs services</caption>
        <thead>
          <tr>
        ...
        <tbody>
          {items.map((item) => (
            <tr key={text(item.service)}>
              <th scope="row">{text(item.service) || "unknown"}</th>
              <td>{text(item.serviceGeneration) || "—"}</td>
...
function health(value: unknown): string {
  const item =
