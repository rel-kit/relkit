...
export function TaskDetailClient() {
  ...
  useEffect(() => {
    if (id === "") return;
    void api
      .taskDefinition<InspectorObject>(id)
      .then((result) => {
        setTask(record(result.task) ?? record(result));
      })
      .catch(() => setError(true));
  }, [api, id]);
  if (error)
    return (
      <main className="route-page">
        <p className="panel route-state" role="alert">
          The task definition is unavailable.
        </p>
      </main>
    );
  if (task === undefined)
    ...
    <main className="route-page">
      <header className="page-heading">
        <div>
          ...
          <h1>{text(task.taskId) || id}</h1>
          <p className="lede">
            Task schema, dependencies, policy, hooks, versions, and worker capabilities.
          </p>
        </div>
        <span className="badge">{text(task.version) || "unversioned"}</span>
      </header>
      <p className="supporting-copy">
        <Link href="/jobs">Back to job definitions</Link>
      </p>
      <ContentTabs
        ...
        items={[
            ...
            content: (
              <JsonPanel
                value={{
                  ...
                  output: task.output,
                  errors: task.errors,
                  schemaHashes: task.schemaHashes,
                }}
              />
            ),
          },
          {
            ...
            content: (
              <JsonPanel value={{ dependencies: task.dependencies, publishes: task.publishes }} />
            ),
          },
          {
            id: "policy",
...
function JsonPanel({ value }: { readonly value: InspectorObject }) {
