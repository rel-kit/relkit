...
const OPENAPI_PATH = "_zsys/v1/openapi.json";
const INSPECTOR_BACKEND_PATH = "/_zsys/backend";

type RouteContext = { readonly params: Promise<{ readonly path: readonly string[] }> };

async function proxy(request: Request, context: RouteContext): Promise<Response> {
  const backend = process.env.ZSYS_BACKEND_URL?.replace(/\/$/, "");
  if (backend === undefined)
    return Response.json({ error: "ZSYS inspector backend is not configured." }, { status: 503 });
