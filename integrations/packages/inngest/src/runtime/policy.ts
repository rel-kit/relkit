...
function mapRetry(value: unknown): number {
  ...
  if (typeof maxAttempts !== "number" || !Number.isSafeInteger(maxAttempts) || maxAttempts < 1)
    throw new TypeError("Inngest retry.maxAttempts is invalid");
  const attempts = maxAttempts;
  if (retry.initialDelay !== undefined && duration(retry.initialDelay) !== 1_000)
    throw new Error("Inngest cannot certify custom retry initialDelay.");
  if (retry.maxDelay !== undefined && duration(retry.maxDelay) !== 30_000)
    throw new Error("Inngest cannot certify custom retry maxDelay.");
  if (retry.factor !== undefined && retry.factor !== 2)
    throw new Error("Inngest cannot certify custom retry factor.");
  if (retry.jitter !== undefined && retry.jitter !== "none")
    throw new Error("Inngest cannot certify retry jitter.");
  return attempts - 1;
}
...
function seconds(value: unknown): number {
  if (typeof value !== "string")
    throw new TypeError("Inngest maxDuration must be a duration string");
  ...
  if (milliseconds < 1 || milliseconds % 1_000 !== 0)
    throw new Error("Inngest maxDuration must be whole seconds");
...
function limit(value: unknown): number {
  ...
  if (
    ...
    !Number.isSafeInteger(concurrency.limit) ||
    concurrency.limit < 1
  )
    throw new TypeError("Inngest concurrency.limit is invalid");
  if (concurrency.key !== undefined)
    throw new Error("Inngest cannot certify keyed task concurrency in this profile.");
