...
test("maps only UTC cron schedules to native Docker triggers", () => {
  ...
  ).toEqual([{ event: "relkit/job/task/v1/build" }, { cron: "0 * * * *" }]);
  expect(() =>
    createInngestFunctionConfig({
      functionId: "relkit-job-task-v1-build",
      eventName: "relkit/job/task/v1/build",
...
test("executes static cron triggers with the declared schedule input", async () => {
  ...
  expect(received).toEqual({
    input: { version: 1, kind: "json", value: { value: 1 } },
...
test("registers a native serve endpoint with the injected task executor", async () => {
  ...
  const worker = runtime.registerWorker({
    definitions: [
      { functionId: "relkit-job-task-v1-build", eventName: "relkit/job/task/v1/build" },
    ],
    executor: { execute: async () => ({ ok: true }) },
  });
  const response = await worker.handler(
    new Request("http://127.0.0.1:3000/api/inngest", {
      method: "PUT",
    ...
    fetch: async (input, init) => {
      ...
      if (url.includes("/events/")) {
        ...
        return Response.json({
          data: eventRuns === 1 ? [{ id: "run-1", status: "Completed", output: { ok: true } }] : [],
        });
      }
...
test("uses the acceptance identity for first-wins event submission", async () => {
  ...
  const first = await runtime.submit(
    {
      ...
      input: { value: 1 },
      operationId: "first",
      acceptanceIdentity: "accepted-once",
      tags: ["one"],
  ...
  expect(payload?.[0]?.data.relkit).toMatchObject({
    acceptanceIdentity: "accepted-once",
    tags: ["one"],
    correlationId: "correlation",
  });
...
test("hashes signing keys for native API authorization and preserves null output", async () => {
  ...
  expect(snapshot(run, metadata)).toMatchObject({
    status: "completed",
    resultAvailability: "available",
    output: null,
  });
...
test("memoizes terminal native retries and indexes the new run", async () => {
  ...
  const runtime = createInngestRuntime({
    ...
    fetch: async (input) => {
      ...
      if (url.includes("/v2/events/event-1/runs"))
        return Response.json({ data: [{ id: "run-1", status: "Completed" }] });
  ...
  await expect(runtime.list({ runId: "run-2" }, context)).resolves.toMatchObject({
    items: [{ runId: "run-2", retryOfRunId: "run-1" }],
  });
  await runtime.close();
});
