...
import { createEffectMqRuntime, type EffectMqNativeClient } from "./src/runtime/index.ts";
...
test("declares the pinned PostgreSQL native adapter and account-free recipe", () => {
  ...
  expect(adapter.localRecipe).toEqual({ integrationId: "effect-mq", recipeId: "effect-mq-docker", recipeVersion: 2 });
...
test("uses native effect-mq receipts and does not create a fallback store", async () => {
  ...
  const native: EffectMqNativeClient = {
    submit: async (request) => {
      ...
      return { accepted: true, runId: "run-1", jobId: request.jobId, taskId: request.taskId, taskVersion: request.taskVersion, acceptedAt: new Date().toISOString() };
    },
    get: async () => ({ accepted: true, runId: "run-1", jobId: "job", taskId: "task", taskVersion: "1", acceptedAt: new Date().toISOString(), buildId: "build", service: "effect-mq", status: "queued", observedAt: new Date().toISOString(), resultAvailability: "pending" }),
    list: async () => ({ items: [], hasMore: false, availability: [] }),
    cancel: async (_runId, operationId) => ({ runId: "run-1", operationId, outcome: "requested" }),
    retry: async (request) => ({ accepted: true, runId: "run-2", jobId: request.jobId ?? "job", taskId: request.taskId ?? "task", taskVersion: request.taskVersion ?? "1", acceptedAt: new Date().toISOString(), retryOfRunId: request.runId }),
  };
  const runtime = createEffectMqRuntime({ native });
  const receipt = await runtime.submit({ jobId: "job", taskId: "task", taskVersion: "1", buildId: "build", input: { value: 1 }, operationId: "op" }, {
    ...context(),
  });
...
function context() {
  return { signal: new AbortController().signal, application: "app", environment: "test", scope: "trusted", service: "effect-mq", serviceGeneration: "generation" };
}
