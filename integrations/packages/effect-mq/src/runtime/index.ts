import type { JobsAdapterRuntime, NativeScheduleOperations } from "@relkit/jobs/adapter";
import {
...
import {
  ...
  type EffectMqWorkerHandle,
  type EffectMqWorkerRegistrationOptions,
} from "./worker.js";
...
export function createEffectMqRuntime(options: EffectMqRuntimeOptions = {}): EffectMqRuntime {
  ...
  const effectiveOptions = {
    ...
    ...(options.readTimeoutMs === undefined && serviceObservation?.readTimeout === undefined
      ...
      : {
          readTimeoutMs:
            options.readTimeoutMs ?? durationToMillis(serviceObservation!.readTimeout!),
        }),
  };
  ...
  const adapter = {
    ...
    capabilities: Object.freeze({
      ...
      capabilities: Object.freeze({
        ...
        observation: {
          ...
          evidence: ["native poll or bounded provider watch"],
        },
        ...
        ...(schedules === undefined
          ? {}
          : { schedules: { support: "native" as const, evidence: ["effect-mq Job.schedule"] } }),
        "durable-sleep": {
          support: "unsupported" as const,
          evidence: ["retryable PostgreSQL profile"],
        },
      }),
    }),
    ...
    observe:
      native.observe ??
      ((request, context) => observeEffectMqRun(native.get, request, context, effectiveOptions)),
  ...
  registrations: Object.freeze([
    {
      ...
      create: ({ connection, behavior }: RuntimeProviderContext) => {
        const databaseUrl =
          optionalText(process.env.RELKIT_EFFECT_MQ_DATABASE_URL) ??
          optionalText(connection.postgresUrl);
        if (databaseUrl === undefined)
          throw new Error("effect-mq postgresUrl is required for a runtime binding");
...
function unboundNative(): EffectMqNativeClient {
  const fail = async (): Promise<never> => {
    throw new Error("RELKIT_EFFECT_MQ_RUNTIME_UNBOUND");
  };
