...
export interface EffectMqScheduleStore {
  readonly listSchedules: (options?: { readonly jobName?: string; readonly group?: string }) => Effect.Effect<ReadonlyArray<EffectScheduleRecord>, unknown, unknown>;
  readonly upsertSchedule: (schedule: EffectScheduleRecord) => Effect.Effect<void, unknown, unknown>;
...
export function createEffectMqScheduleOperations(options: EffectMqScheduleBridge): NativeScheduleOperations {
  const listRecords = async (query: Readonly<Record<string, unknown>>): Promise<readonly EffectScheduleRecord[]> =>
    options.run(options.store.listSchedules({
  ...
  return {
    ...
    get: async (id, context) => {
      ...
      const record = rows.find((entry) => String(entry.key) === scheduleKey(options.jobName, id) && inScope(entry, context.scope));
      return record === undefined ? { outcome: "unavailable" } : { outcome: "available", schedule: fromNative(record, options) };
    },
    upsert: async (definition, context) => {
      const schedule = definition as ScheduleDefinition & { readonly metadata?: Readonly<Record<string, string>> };
      const record = toNative(schedule, options, context);
      await options.run(options.store.upsertSchedule(record));
      return { operationId: context.operationId ?? "effect-mq-schedule", scheduleId: schedule.id, outcome: "updated" };
    },
    ...
    delete: async (id, context) => {
      const removed = await options.run(options.store.removeSchedule(ScheduleKey(scheduleKey(options.jobName, id))));
      return { operationId: context.operationId ?? "effect-mq-schedule", scheduleId: id, outcome: removed ? "deleted" : "requested" };
    },
  };
}
...
function toNative(
  ...
  if (schedule.overlap !== undefined && schedule.overlap !== "allow") {
    throw new Error(`effect-mq schedule "${schedule.id}" does not support overlap=${schedule.overlap}`);
  }
  if (schedule.misfire !== undefined && schedule.misfire !== "skip") {
    throw new Error(`effect-mq schedule "${schedule.id}" does not support misfire=${schedule.misfire}`);
  }
  ...
  if (nextRunAt === undefined) throw new Error(`effect-mq schedule "${schedule.id}" has an invalid recurrence`);
  return {
...
function fromNative(record: EffectScheduleRecord, options: EffectMqScheduleBridge) {
  ...
  const input = wrapped !== undefined && Object.hasOwn(wrapped, "input") && Object.hasOwn(wrapped, "metadata")
    ? wrapped.input
    : payload !== undefined && Object.hasOwn(payload, "input") ? payload.input : record.payload;
  const definition: ScheduleDefinition = record.cron !== undefined
...
function everyDuration(milliseconds: number | undefined): Extract<ScheduleDefinition["every"], string> {
  const value = Math.max(1, milliseconds ?? 1_000);
  return (value % 1_000 === 0 ? `${value / 1_000} seconds` : `${value} milliseconds`) as Extract<ScheduleDefinition["every"], string>;
}
...
function inScope(record: EffectScheduleRecord, scope: string): boolean {
  ...
  return metadata !== null && typeof metadata === "object" && !Array.isArray(metadata) && metadata.scope === scope;
}
