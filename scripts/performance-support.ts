...
export const load = async (path: string): Promise<R> =>
  (await import(pathToFileURL(resolve(root, path)).href)) as R;
...
function summarize(iterations: number, totalMs: number, durations: number[], warmups: number): R {
...
export function makeDescriptors(
  count: number,
  defineFunction: R["defineFunction"],
  z: R["z"],
): unknown[] {
  ...
  return Array.from({ length: count }, (_, index) =>
    defineFunction({
      ...
      input: schema,
      output: schema,
      handler: (value: R) => value,
    }),
  );
}

export const manifest = (graphHash: string): R => ({
  contractVersion: 1,
  generatorVersion: 1,
  graphHash,
  functions: {},
...
export function makeRoutePlan(targetFunctionId: string): R {
...
export function invocationOptions(source: string): R {
