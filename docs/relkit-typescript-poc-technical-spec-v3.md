# RelKit TypeScript POC — Technical Architecture and Implementation Specification
...
This document is the implementation specification for the first RelKit proof of concept. It is written for engineers who will build the framework, including junior engineers who need explicit file responsibilities, dependencies, inputs, outputs, expected behavior, test commands, and completion criteria.
...
This revision replaces earlier RelKit POC specifications wherever they conflict with it.
...
RelKit manages these application concepts:
...
Application developers remain free to use ordinary libraries from their own code. RelKit does not inspect or model those libraries in the application graph.

### 2.3 No public Effect requirement

Effect is the internal execution kernel of RelKit. An application developer writes normal TypeScript:
...
The public schema boundary accepts Standard Schema-compatible schemas. The scaffold uses `@relkit/schema`; it is not Effect Schema.
...
provider-neutral deployment plan
      ↓
Pulumi program generated in memory or in .relkit/generated
      ↓
...
A RelKit developer should learn five ideas.
...
.relkit/generated/runtime.manifest.ts
.relkit/generated/openapi.json
.relkit/generated/client.ts
.relkit/generated/diagnostics.json
```
...
The default `relkit dev` command starts a supervisor process that owns two child generations:
...
| ----------------- | ---------------------------------------------------------------- |
| `@relkit/app`       | `defineApp` and re-exports for common descriptor factories       |
| `@relkit/schema`    | supported schema builder plus Standard Schema bridge             |
| `@relkit/config`    | environment declaration DSL                                      |
| `@relkit/functions` | function, error, and reference contracts                         |
| `@relkit/routes`    | HTTP route declaration and mapping DSL                           |
| `@relkit/jobs`      | job and schedule declarations                                    |
| `@relkit/events`    | event contracts, selectors, `onEvent`, and publishing references |
| `@relkit/buckets`   | logical bucket declarations                                      |
| `@relkit/cache`     | typed cache declarations                                         |
| `@relkit/tools`     | function-to-tool declarations                                    |
| `@relkit/agents`    | agent declarations and model profiles                            |
| `@relkit/testing`   | application test harness and provider fakes                      |

`@relkit/app` SHOULD re-export the common surface so a generated project normally needs only:
...
| Package                 | Responsibility                                                       |
| ----------------------- | -------------------------------------------------------------------- |
| `@relkit/contracts`       | JSON values, branded IDs, source locations, versioned protocol types |
| `@relkit/diagnostics`     | structured errors and warnings                                       |
| `@relkit/graph`           | normalized graph schema, hashing, sorting, compatibility diff        |
| `@relkit/compiler`        | discovery, evaluation, normalization, code generation                |
| `@relkit/engine`          | invocation protocol, lifecycle, concurrency, cancellation            |
| `@relkit/runtime-effect`  | Effect runtime, Layers, internal error causes, loggers, tracing      |
| `@relkit/runtime-hono`    | graph-driven Hono materialization                                    |
| `@relkit/providers-local` | local bucket, cache, jobs, events, and fake model providers          |
| `@relkit/observability`   | request records, logs, traces, storage, query protocol               |
| `@relkit/supervisor`      | candidate generation, readiness, switching, draining                 |
| `@relkit/inspector-api`   | versioned inspector HTTP/SSE protocol                                |
| `@relkit/deploy`          | provider-neutral deployment plan                                     |
| `@relkit/deploy-pulumi`   | Pulumi Automation API integration                                    |
| `@relkit/cloud-aws`       | AWS runtime providers and Pulumi resource mapping                    |
| `@relkit/cli`             | `relkit` commands and terminal UX                                      |
| `create-relkit`           | new-project generator                                                |

### 6.4 External implementation dependencies

These dependencies are hidden behind RelKit packages in generated applications:

| Dependency                                   | Used by                                                           |
| -------------------------------------------- | ----------------------------------------------------------------- |
| `effect`                                     | internal execution, lifecycle, logs, tracing, config, concurrency |
| `hono`                                       | internal HTTP runtime                                             |
| `zod` or equivalent supported implementation | `@relkit/schema` default builder                                    |
...
```ts
export interface RelkitSchema<TInput = unknown, TOutput = TInput> {
  readonly "~standard": {
    readonly version: 1;
...
Other Standard Schema-compatible libraries MAY be accepted if RelKit can obtain or generate the required JSON Schema. Effect Schema is not required or exposed by the scaffold.
...
> extends DescriptorBase<"function", Id> {
  readonly input: RelkitSchema<unknown, Input>;
  readonly output: RelkitSchema<unknown, Output>;
...
> extends DescriptorBase<"event", Id> {
  ...
  readonly payload: RelkitSchema<unknown, Payload>;
...
> {
  readonly profile?: string;
  readonly key: RelkitSchema<unknown, Key>;
  readonly value: RelkitSchema<unknown, Value>;
...
## 8. Effect as the internal RelKit kernel
...
All RelKit framework and CLI logs MUST enter the Effect logging system. Application code logs through `ctx.log`.
...
export default defineEnv({
  RELKIT_ENV: env.literal("development", "test", "production").default("development"),
...
- `relkit env check` validates the active environment and exits non-zero on missing or malformed values.
- `relkit env example` generates `.env.example` deterministically without overwriting user edits unless `--write` is provided.
- `relkit env explain` prints type, requirement rules, default presence, and description without printing a secret.
- `relkit env list` prints names and status only.
...
```ts
// src/app.ts
import { defineApp, localProviders, testProviders, awsProviders } from "@relkit/app";
import env from "./env";
...
export default defineApp({
  ...
  providers: {
    development: localProviders({
      stateDirectory: ".relkit/state",
      observabilityDirectory: ".relkit/observability",
    }),
...
Routes and functions do not have providers. Hono and the execution engine are RelKit runtime implementation.
...
`relkit.config.ts` contains tooling configuration rather than application behavior:

```ts
import { defineConfig } from "@relkit/cli";
...
The compiler uses the TypeScript compiler API to find files that might export RelKit descriptors. Candidate indicators include:

```text
imports from @relkit/*
known factory identifiers
default exports
RELKIT_DESCRIPTOR symbol access
...
```text
RELKIT_CONVENTION_DIRECTORY
RELKIT_CONVENTION_SUFFIX
RELKIT_CONVENTION_EXPORT
RELKIT_CONVENTION_MULTIPLE_KINDS
RELKIT_CONVENTION_ID_STYLE
```
...
```text
RELKIT_DUPLICATE_ID
RELKIT_MISSING_TARGET
RELKIT_ROUTE_COLLISION
RELKIT_SCHEMA_UNAVAILABLE
RELKIT_MAPPING_INCOMPATIBLE
RELKIT_EVENT_SELECTOR_EMPTY
RELKIT_EVENT_TARGET_INCOMPATIBLE
...
The local provider uses append-only files and atomic renames under `.relkit/state/jobs`. It MUST support:
...
retry/DLQ       → SQS redrive plus RelKit policy validation
...
RelKit can react to any event that is explicitly published into its event capability, including selected RelKit system events. It does not infer changes made by unrelated application libraries.
...
retry/DLQ              → SQS redrive plus RelKit policy
```
...
```text
GET /_relkit/v1/requests
GET /_relkit/v1/requests/:requestId
GET /_relkit/v1/logs
GET /_relkit/v1/traces
GET /_relkit/v1/traces/:traceId
```
...
The inspector is a read-oriented development product over versioned RelKit APIs. It MUST NOT:
...
## 21. Creating a new RelKit project
...
7. write `package.json` with exact compatible RelKit versions;
8. generate `relkit.config.ts`;
...
```json
{
  ...
  "dependencies": {
    "@relkit/app": "<compatible-version>",
    "@relkit/config": "<compatible-version>",
    "@relkit/schema": "<compatible-version>"
  },
  "devDependencies": {
    "@types/bun": "<compatible-version>",
    "@relkit/cli": "<compatible-version>",
    "@relkit/testing": "<compatible-version>",
...
route:     GET http://localhost:3000/hello?name=RelKit
```
...
JSON output
all generated projects pass bun install, relkit check, tsc, bun test, and relkit build
...
`@relkit/deploy-pulumi` converts the deployment plan into a Pulumi program. Generated files may be materialized under:

```text
.relkit/generated/pulumi/
├── Pulumi.yaml
├── index.ts
└── plan.json
```
...
```text
Pulumi project name → normalized RelKit app ID
Pulumi stack name   → explicit CLI stack
resource logical name → stable graph ID-derived name
resource tags       → app ID, stack, graph hash, managed-by=relkit
```
...
`@relkit/cloud-aws` SHOULD define reusable component resources:
...
Components encapsulate defaults while exposing an escape hatch only through explicitly supported RelKit configuration. Arbitrary Pulumi callbacks are not part of the POC application model.

### 22.6 Initial AWS mapping

| RelKit concept          | AWS resource                                                          |
| --------------------- | --------------------------------------------------------------------- |
...
`relkit build` produces:

```text
.relkit/build/server/
.relkit/build/manifest.json
.relkit/build/application.graph.json
.relkit/build/openapi.json
.relkit/build/Dockerfile
```
...
exclude local .env files and .relkit/state
```
...
`relkit deploy up` requires a successful compile and may require an interactive confirmation when destructive or security-sensitive changes are present. CI uses explicit non-interactive flags.

### 22.10 State backend

RelKit does not invent a second infrastructure state system. The user configures a Pulumi backend. Supported POC paths:
...
### 23.5 `@relkit/testing` public harness
...
Tests MUST NOT share `.relkit/state` or ports. The harness deletes temporary directories on success and retains them on failure when `RELKIT_KEEP_TEST_STATE=1`.
...
1. run the locally built `create-relkit` package in a temporary parent directory;
...
6. run `bun run test`;
7. run `bun run build`;
8. start `relkit dev` on dynamic ports;
...
```text
typescript
chosen schema implementation inside @relkit/schema
...
4. Expose a familiar `z` builder from `@relkit/schema`.
...
| Path                                               | Responsibility                  |
| -------------------------------------------------- | ------------------------------- |
| `packages/compiler/src/config-loader.ts`           | load `relkit.config.ts`           |
...
@relkit/schema
@relkit/observability contract stubs
```
...
@relkit/graph
@relkit/schema
@relkit/observability contracts
```
...
| `packages/cli/src/commands/dev.ts`         | `relkit dev` orchestration                |
...
Deliver the user-facing commands and make `bunx create-relkit` produce a complete working project.
...
| `packages/create-relkit/src/index.ts`    | generator entry               |
| `packages/create-relkit/src/options.ts`  | flags and defaults            |
| `packages/create-relkit/src/generate.ts` | temporary-dir generation      |
| `packages/create-relkit/src/validate.ts` | name/path validation          |
...
3. Implement `relkit check`, `build`, `start`, `graph`, `env`, and `doctor`.
...
```bash
bun test packages/cli packages/create-relkit tests/generator
...
relkit deploy commands
preview/up/destroy reports
```

#### Expected behavior

`relkit deploy preview` shows a deterministic plan without changes. `relkit deploy up` creates a reachable service and managed capabilities. Running it again with no graph change is a no-op. Moving source files without changing IDs does not replace resources.
...
relkit deploy preview --stack ci-<id> --non-interactive
```
...
```text
packed RelKit packages
packed create-relkit package
...
- `bunx create-relkit@latest my-app` produces a complete project.
...
Public schema               Standard Schema via @relkit/schema by default
...
Project creation            bunx create-relkit@latest <name>
