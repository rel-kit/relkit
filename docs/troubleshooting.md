...
| A connected binding appears as infrastructure    | Provider source and role projections in the Inspector and deployment preview                             | Pass the configured adapter directly. Only an explicit infrastructure wrapper such as `aws(adapter)` may emit lifecycle or access operations.                            |
| A Docker-backed binding will not start           | `relkit local status`, Docker availability, recipe health, and the project lease                         | Start Docker, repair the reported recipe, or use `dev --local=off` with every required connected/infrastructure value supplied.                                          |
| Release validation rejects a local-only binding  | Provider source in the graph and `relkit check` diagnostic                                               | Add a connected adapter configuration or supported infrastructure wrapper. Docker recipes never become deployment resources.                                             |
...
`RELKIT_DEPLOY_CONFIGURATION_MISSING`, `RELKIT_DEPLOY_PROFILE_UNSUPPORTED`,
`RELKIT_DEPLOY_ROLE_MISSING`, `RELKIT_DEPLOY_ROLE_INVALID`, and
