...
| Environment is incomplete                    | `zsys env check`, `zsys env list`, `zsys env explain NAME`                                           | Supply the same declared keys from the active pipeline. Do not add provider branches or declare the reserved `ZSYS_ENV` key.                                  |
...
| Restart fails with `generation-1` already exists | `lsof -nP -iTCP:3000 -sTCP:LISTEN` and `.zsys/generated`                                         | Stop the old dev process and rerun; current CLI runs use isolated `.dev-*` generation directories, so future restarts do not reuse stale candidates.          |
...
| Inspector UI changes are not visible         | `ps`, `lsof -nP -iTCP:3210 -sTCP:LISTEN`, and the generated app's linked `@zsys/cli`                  | Run a source-backed local CLI; linked checkouts prefer `apps/inspector`, while published installs use the packaged inspector.                                |
