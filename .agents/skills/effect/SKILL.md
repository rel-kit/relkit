...
Use this skill to identify the relevant Effect APIs, then check their local source
and usages before implementing them:

- the nearest `AGENTS.md` and any project-local Effect practices doc
- always inspect the corresponding implementation, tests, and examples in the
  read-only `repos/effect` checkout for each relevant API or pattern (for
  example, inspect Layer usage there when choosing `Layer`); search with
  `rg --no-ignore` because `repos/*` is ignored
- check the project-pinned `effect` version and installed package source to
  verify compatibility with the local upstream checkout
- if the checkout lacks a relevant file, note the gap and use the installed
  package source; consult current upstream source if both remain inconclusive
